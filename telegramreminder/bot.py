...
текст дата(только тек месяц) время - на число текущего месяца главное не спутать с на сегодня("В") 
...
+ периодическое:
 ...
 Как делать:
    ...
    Вариант с парсингом:
        ...
        если в конце слово - периодичность

        далее всегда ищу время - либо одно, либо 2 числа ВСЕГОДА ПОСЛЕ "В"

        далее пытаюсь найти месяц - либо число, либо число + слово

        ЛИБО:

        Далее будет день словом

        ЛИБО : ничего - это сегодня

        Все, что осталось - текст напоминания


ВАЛИДАЦИЯ ОБЯЗАТЕЛЬНА, опция - отмена ввода
